...
class IBMEA:
    ...
    def __init__(self):
        ...
        self.args.device = self.device  # 将设备信息传递给args
    ...
    def parse_options(parser):
        ...
        # === 新增CLIP相关参数 ===
        parser.add_argument(
            "--use_clip", action="store_true", default=False, 
            help="use CLIP for feature extraction"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--freeze_clip", action="store_true", default=True,
            help="freeze CLIP parameters during training"
        )
        parser.add_argument(
            "--use_text_vib", action="store_true", default=False,
            help="apply VIB to CLIP text features"
        )
        parser.add_argument(
            "--clip_cache_dir", type=str, default="data/clip_cache",
            help="directory to cache CLIP preprocessed data"
        )
        parser.add_argument(
            "--max_clip_entities", type=int, default=None,
            help="maximum number of entities to process with CLIP (for debugging)"
        )
        parser.add_argument(
            "--clip_batch_size", type=int, default=32,
        ...
        parser.add_argument("--Beta_t", type=float, default=0.001, help="text beta for CLIP")
        parser.add_argument(
    ...
    def print_summary(self):
        ...
        if self.args.use_clip:
            ...
            print("CLIP freeze:\t", self.args.freeze_clip)
            print("Text descriptions:\t", len(self.text_descriptions) if self.text_descriptions else 0)
            print("Processed images:\t", len(self.processed_images) if self.processed_images else 0)
        print("-------------------------")
    ...
    def init_data(self):
        ...
        if self.args.use_clip:
            ...
            # 创建缓存目录
            cache_dir = self.args.clip_cache_dir
            os.makedirs(cache_dir, exist_ok=True)
            
            # 生成缓存文件名
            dataset_name = os.path.basename(file_dir)
            cache_file = os.path.join(cache_dir, f"{dataset_name}_clip_data.pkl")
            
            # 尝试从缓存加载
            cached_data = load_clip_cache(cache_file)
            if cached_data is not None:
                self.text_descriptions = cached_data.get('text_descriptions', {})
                self.processed_images = cached_data.get('processed_images', {})
            else:
                # 加载原始数据
                self.text_descriptions, self.image_paths = load_clip_data(
                    file_dir, self.ent2id_dict
                )
                
                # 预处理图像（如果没有图像路径，则创建空字典）
                ...
                if self.image_paths:
                    ...
                    try:
                        # 需要先初始化模型才能获取预处理器
                        import clip
                        _, clip_preprocess = clip.load(self.args.clip_model, device=device)
                        self.processed_images = preprocess_clip_images(
                            self.image_paths, 
                            clip_preprocess, 
                            device,
                            self.args.max_clip_entities
                        )
                ...
                save_clip_cache(cache_data, cache_file)
            
            print(f"Loaded {len(self.text_descriptions)} text descriptions")
            print(f"Loaded {len(self.processed_images)} preprocessed images")
    ...
    def prepare_clip_batch_data(self, entity_indices):
        """
        为给定的实体索引准备CLIP批次数据
        """
        if not self.args.use_clip:
            return None, None
            
        batch_texts, batch_images = create_clip_batch_data(
            entity_indices, 
            self.text_descriptions or {}, 
            self.processed_images or {}, 
            self.device
        )
    ...
    def semi_supervised_learning(self):
        with torch.no_grad():
            # 准备CLIP数据
            batch_texts, batch_images = self.prepare_clip_batch_data(self.input_idx)
            
            gph_emb, img_emb, rel_emb, att_emb, text_emb, name_emb, char_emb, joint_emb = (
            ...
            del gph_emb, img_emb, rel_emb, att_emb, text_emb, name_emb, char_emb, joint_emb
    ...
    def train(self):
        ...
        use_text_vib = self.args.use_text_vib
        ...
        Beta_t = self.args.Beta_t  # CLIP文本特征权重
        ...
        for epoch in range(self.args.epochs):
            ...
            for si in np.arange(0, self.train_ill.shape[0], bsize):
                ...
                # === CLIP文本损失 ===
                if self.args.use_clip and text_emb is not None and use_text_vib:
                    text_bce_loss = self.criterion_nce(
                        text_emb, self.train_ill[si : si + bsize]
                    )
                    if hasattr(self.multimodal_encoder, 'text_kld_loss'):
                        text_kld_loss = self.multimodal_encoder.text_kld_loss
                        loss_T = text_bce_loss + Beta_t * text_kld_loss
                        print(
                            " text_bce_loss: {:f}, text_kld_loss: {:f}, Beta_t: {:f}".format(
                                text_bce_loss, text_kld_loss, Beta_t
                            ),
                            end="",
                        )
                    else:
                        loss_T = text_bce_loss
                        print(
                            " text_bce_loss: {:f}".format(text_bce_loss),
                            end="",
                        )
                ...
                loss_all = sum(loss_list) if loss_list else torch.tensor(0.0, device=device, requires_grad=True)
